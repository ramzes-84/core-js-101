...
function getRectangleString(width, height) {
  ...
  const middleLine2 = middleLine.repeat(height - 2);
  const lastLine = `└${('─').repeat(width - 2)}┘\n`;
  return firstLine + middleLine2 + lastLine;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (code > 96 && code <= 109) {
      ...
      letter = String.fromCharCode(96 + (13 - (122 - code)));
    } else if (code > 64 && code <= 77) {
      letter = String.fromCharCode(code + 13);
    } else if (code > 77 && code <= 90) {
      letter = String.fromCharCode(64 + (13 - (90 - code)));
    } else {
